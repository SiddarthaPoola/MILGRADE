...
//     ipAddress: ipAddress,
//     HTTP_PORT: HTTP_PORT // added these two lines
...
const app = express();
const server = http.createServer(app);
const io = new Server(server, { cors: { origin: "*" } }); // Allowing CORS for mobile connections
...
function handleMessage(hexArray) {
    ...
    case 0x0002:
      ...
      case 0x0003:
        ...
        if (passwordAck === 0x01) {
          ...
          // Add user-friendly popup notification for failed password change
          io.emit("showPopup", {
            title: "Password Change Failed",
            message: "The old password you entered was incorrect. Please try again with the correct current password.",
            type: "error"
          });
        } else {
          // Handle unexpected values
          ...
          // Add user-friendly popup notification for unknown errors
          io.emit("showPopup", {
            title: "Password Change Error",
            message: "An unexpected error occurred while changing your password. Please try again later.",
            type: "error"
          });
        }
        break;
  ...
  if (messageLength === 0x0007) {
    ...
    if (lastByte === 0x00) {
      ...
      // Stay in Auto Page
      console.log("Auto mode acknowledgment received.Navigating to Auto Page.");
      io.emit("navigateToAutopage");// No action needed, stay in Auto Page
    } else {
      // Handle unexpected values
...
// Add this at the top with other variable declarations
  ...
  case 0x000C: // New data format
      ...
      // Extract relay statuses from the message
      const relayStatus = hexArray.slice(6); 
  
      // Convert the extracted relay data to a hexadecimal string
      const relayStatusHex = relayStatus.map(byte => byte.toString(16).padStart(2, '0')).join('');
  
      console.log("Processed Relay Status:", relayStatusHex);
  
      // Emit the hexadecimal relay status to the frontend
      io.emit("relayStatusUpdate", relayStatusHex);
  
      result = "Relay operation ACK received";
      break;
  
  default:
      result = `Unknown operation code: 0x${operationCode.toString(16).padStart(4, "0")}`;
...
//       const hexArray = [];
// console.log("MSG from MCU", asciiString.length);
...
//   }
// }
function startUDPSocket(remotePort, ipAddress) {
  if (!udpSocket) {
    udpSocket = dgram.createSocket("udp4");
    ...
    udpSocket.on("message", (msg, rinfo) => {
      ...
      // Convert raw binary data into an array of hex values
      const hexArray = Array.from(msg);

      console.log("Raw Hex Array:", hexArray); // Example: [0x00, 0x06, 0x00, 0x01, 0x00, 0x00]
...
io.on("connection", (socket) => {
  ...
  socket.on("setUdpConfig", (config) => {
    ...
    startUDPSocket(REMOTE_UDP_PORT, MICROCONTROLLER_IP);
    ...
    console.log(
      `UDP configuration set: IP=${MICROCONTROLLER_IP}, Remote Port=${REMOTE_UDP_PORT}}`
    );
  });
  ...
  let message = [0x00, 0x10, 0x00, 0x03, 0x00, 0x00]; // Initial header (Size: 16 bytes, Opcode: 0x0003)

  // Append old password (5 bytes) - convert each character to hexadecimal
  for (let i = 0; i < 5; i++) {
    // If the character is a digit, convert it to its numeric value (0-9)
    // Otherwise, use its ASCII value
    const char = oldPassword.charAt(i);
    const hexValue = /^\d$/.test(char) ? parseInt(char, 10) : oldPassword.charCodeAt(i);
    message.push(hexValue);
  }

  // Append new password (5 bytes) - convert each character to hexadecimal
  for (let i = 0; i < 5; i++) {
    // If the character is a digit, convert it to its numeric value (0-9)
    // Otherwise, use its ASCII value
    const char = newPassword.charAt(i);
    const hexValue = /^\d$/.test(char) ? parseInt(char, 10) : newPassword.charCodeAt(i);
    message.push(hexValue);
  }
  ...
  socket.on("saveNetworkSettings", (settings) => {
    ...
    let message = [0x00, 0x14, 0x00, 0x06, 0x00, 0x00]; // Size: 16 bytes, Opcode: 0x0006
  ...
  socket.on("autoManualToggleAttempt", (currentPage) => {
    ...
    if (currentPage === "homePage") {
      ...
      message = [0x00, 0x07, 0x00, 0x08, 0x00, 0x00, 0x00]; //  0x00, 0x07, 0x00, 0x08, 0x00, 0x00, 0x01
    } else {
...
socket.on('relayButtonClick', (data) => {
  ...
  const message = [
    ...
    0x00, // Reserved byte
    0x00, // Reserved byte
    relayNumber, // Relay number (1 to N-1)
...
server.listen(HTTP_PORT, "0.0.0.0", () => {
 ...
 // opn(`http://${ipAddress}:${HTTP_PORT}`); this
});